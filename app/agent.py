...
def triage_agent(state: AgentState):
    ...
    # Simple prompt to determine intent
...
def booking_specialist(state: AgentState):
    ...
    # Extract entities (date, time, email)
    prompt = ChatPromptTemplate.from_messages([
        ("system", f"You are a Booking Specialist. The current date is {datetime.date.today()}. Extract the requested date, time, and email from the conversation. If missing, ask the user for them. Respond in JSON format: {{\"date\": \"YYYY-MM-DD or relative like 'tomorrow'\", \"time\": \"HH:MM\", \"email\": \"user@example.com\", \"message\": \"Message to user\"}}. If you have all info, set message to 'READY'."),
        MessagesPlaceholder(variable_name="messages")
    ])
...
# We will compile it with a checkpointer in the Streamlit app
